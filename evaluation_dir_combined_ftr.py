...
from train_combined_ftr import mini_batches
...
def construct_model(data, params):
    ...
    ids_train, labels_train, ftr_train, msg_train, code_train = training
    ids_test, labels_test, ftr_test, msg_test, code_test = testing

    # preprocessing features
    ftr_train, ftr_test = preprocessing.scale(ftr_train), preprocessing.scale(ftr_test)
    ...
    batches_train = mini_batches(X_msg=pad_msg_train, X_code=pad_code_train, X_ftr=ftr_train, Y=labels_train)
    batches_test = mini_batches(X_msg=pad_msg_test, X_code=pad_code_test, X_ftr=ftr_test, Y=labels_test)
    ...
    model = DefectNetCombine(params, ftr_train.shape[1])
...
def eval_dir(dir, data, model):
    ...
    for batch in data:
        ...
        if torch.cuda.is_available():
            pad_msg, pad_code, pad_ftr = torch.tensor(pad_msg).cuda(), torch.tensor(
                pad_code).cuda(), torch.cuda.FloatTensor(pad_ftr)
        else:
            pad_msg, pad_code, pad_ftr = torch.tensor(pad_msg).long(), torch.tensor(pad_code).long(), torch.tensor(
                pad_ftr).float()
        if torch.cuda.is_available():
